...
const quotes = {
  ...
  quote2:
    "At the end of the day I say to myself, 'Did I make a difference?' I hope the answer is always yes.",
  ...
  quote6:
    "Listen<br>I wish I could tell you it gets better.<br> But it doesn't get better.<br>You get Better.",
  quote7:
    "Sometimes you never know the value of a moment until it becomes a memory.",
  quote8:
    "I'll tell you a secret. Something they don;t teach you in your temple. The Gods envy us. They envy us because we're mortal, because any moment might be our last. Everything is more beautiful because we're doomed. You will never be lovelier than you are right now. We will never be here again.",
  quote9:
    "If I quit now, I will soon be back to where I started and when I started, I was desperately wishing to be where I am now.",
  quote10:
    "Just because you took longer than others, doesn't mean you failed.<br> Remember that.",
  quote11:
  ...
  quote15:
    "Stop waiting for Friday, for summer, for someone to fall in love with you, for life. Happiness is achieved when you stop waiting for it and make the most of the moment you're in right now.",
  quote16:
  ...
  quote21:
    "Computer are like Old Testament gods: <br>lots of rules and no mercy.",
  quote22:
    "The night is far spent, the day is at hand;<br> let us therefore cast off the works of darkness, and let us put on the armor of light.",
  quote23:
    "The release of atomic energy has not created a new problem. It has merely made more urgent the necessity of solving an existing one.",
  quote24:
    "Be extremely subtle, even to the point of formlessness. Be extremely mysterious, even to the point of soundlessness. Thereby you can be the director of the opponent's fate.",
  quote25:
    "Take the risk of thinking for yourself,<br>much more happiness, truth, beauty, and wisdom<br> will come to you that way.",
  haiku1: "The wren<br>Earns his living<br>Noiselessly.",
  haiku2:
    "Winter seclusion -<br> Listening, that evening,<br>To the rain in the mountain.",
  haiku3: "Summer night -<br>even the stars<br>are whispering to each other.",
  haiku4:
    "Over the wintry<br>forest, winds howl in rage<br>with no leaves to blow.",
  haiku5:
    "Toward those short trees<br>We saw a hawk descending<br>On a day in spring.",
  haiku6:
    "Sparrow's child<br>out of the way, out of the way!<br>the stallion's coming through.",
  haiku7:
    "I kill an ant<br>and realize my three children<br>have been watching.",
  haiku8: "The lamp once out<br>Cool stars enter<br>The window frame.",
  haiku9:
    "In the cicada's cry<br>No sign can foretell<br>How soon it must die.",
  haiku10:
    "First autumn morning<br>the mirror I stare into<br>shows my father's face.",
  haiku11:
    "Now my tendrilled soul,<br>Has found its pergola-- Christ--<br>To wind its way up....",
  haiku12:
    "Won't you come and see<br>loneliness? Just one leaf<br>from the kiri tree.<br>Fallen sick on a journey,<br>In dreams I run wildly<br>Over a withered moor.",
  haiku13:
    "If I had the knack<br>I'd sing like<br>Cherry flakes falling<br>Summer grasses,<br>All that remains<br>Of soldiers' dreams",
  haiku14:
    "Clouds come from time to time<br>and bring to men a chance to rest<br>from looking at the moon.",
  haiku15:
    "Hold fast to dreams<br>For if dreams die<br>Life is a broken-winged bird<br>That cannot fly.<br>Hold fast to dreams<br>For when dreams go<br>Life is a barren field<br>Frozen with snow. "
};
const words = {
  blackice: [
    "FRIED",
...
$(document).ready(function () {
  ...
  $("li").click(function () {
    ...
    var typing_effect = setInterval(function () {
      ...
      if (
        undefined != readings.title &&
        counter_title <= readings.title.length
      ) {
        reading += readings.title.charAt(counter_title);
        $("#title").text(reading);
        counter_title++;
      } else if (
        undefined != readings.password &&
        counter_password <= readings.password.length
      ) {
        ...
        undefined != readings.attempts &&
        counter_attempts <= readings.attempts.length + 4
      ) {
  ...
  function hacking_interface() {
    ...
    (function getWords() {
      ...
      for (var x = 0; x <= randomNumber; x++) {
        ingame_words.push(
          player_selected_difficulty[
            getRandomInt(0, player_selected_difficulty.length - 1)
          ]
        );
      }
    ...
    function willUseCorrectWord(rowName) {
      if (
        correctWordUsed === false &&
        wordToNumber(rowName) != Object.keys(rows).length
      ) {
        if (Math.random() <= 0.1) {
    ...
    function willUseRandomWord(rowName) {
      if (
        ingame_words.length + wordToNumber(rowName) >=
        Object.keys(rows).length + 1
      ) {
        //if there is too few rows for words
        return true;
      } else if (ingame_words[0] == undefined || ingame_words[0] == null) {
        //if there is no more words to choose from
        return false;
      } else {
    ...
    function addHTML(row) {
      ...
      return row.map(function (value) {
        if (value.length == 1) {
          ...
          value =
            "<span id = 'dud' data-dud = '" + value + "'>" + value + "</span>";
        } else {
          //if its a word
          value =
            "<span id = 'word' data-word = '" +
            value +
            "'>" +
            value +
            "</span>";
    ...
    function createRow(row, theRowName) {
      ...
      function checkWidth() {
        $("#ignore").html(
          "<span id = 'Width'>" + resultantRow.join("") + "</span>"
        );
        widthOfRow = Math.floor($("#Width").width());
      }
      while (widthOfRow <= x_length) {
        resultantRow = maximizeRow(
          resultantRow,
          useCorrectWord,
          useDud,
          useRandomWord
    ...
    for (var b in rows) {
      $(".hacking-screen").append(
        "<p id = 'rows'>" + rows[b].join("") + "</p>"
      );
    }
    inGameWordsThatAreInActualGame.splice(
      inGameWordsThatAreInActualGame.indexOf(correct_word),
      1
    );
    if ($(".hacking-screen").height() >= 310) {
      $(".hacking-screen").html(
        "<h2 class = 'text-center'>An error occured displaying the screen.</h2>"
      );
    }
    if (sayCorrectWord) {
  ...
  function inputWin(word) {
    ...
    $(".input-screen").append(
      "<p style = 'order :1'> &gt; Please wait while system is accessed.</p>"
    );
  ...
  $(".hacking-screen").on("click", "#word", function () {
    if ($(this).text() == correct_word) {
      //if the player chose the correct word
      $("#pg")[0].play();
      inputWin($(this).text());
      var dotts = "•",
        readings = "&gt; ";
      ...
      //otherwise the player must have choosen an incorrect word
      $("#pb")[0].play();
      tries--;
      if (tries !== 0) {
        (function changeTries() {
          $(".title-screen").html(
            "<p class = 'text-center' id = 'GREEN IMPERIAL (TM) HACKING PROTOCOL</p>"
          ); //title-reading
          $(".title-screen").append(
            "<p id = 'enter-password'>ENTER PASSWORD :</p>"
          ); //enterpassword
          $(".title-screen").append(
            "<p id = 'password-attempts'>Attempts remaining --> " +
              tries +
              " " +
              dots(tries) +
              "</p><br>"
  ...
  $(".hacking-screen").on("click", "#dud", function () {
    ...
    if (Math.random() >= 0.2) {
      (function deleteARandomWord() {
        var wrd = $(".hacking-screen").find(
          "[data-word = '" +
            falseWords[getRandomInt(0, falseWords.length - 1)] +
            "']"
        );
        wrd.text(dotsForDud(wrd.text()));
        inGameWordsThatAreInActualGame.splice(
          inGameWordsThatAreInActualGame.indexOf(wrd.attr("data-word")),
          1
        );
        ...
        $(".input-screen").append(
          "<p order = '" + verticalAlign + "'>&gt; Dud removed.</p>"
        );
      })();
    } else {
      (function replenishAttempts() {
        tries = 4;
        $(".title-screen").html(
          "<p class = 'text-center' id = 'title'>GREEN IMPERIAL (TM) HACKING PROTOCOL</p>"
        ); //title-reading
        $(".title-screen").append(
          "<p id = 'enter-password'>ENTER PASSWORD :</p>"
        ); //enterpassword
        ...
        $(".input-screen").append(
          "<p order = '" + verticalAlign + "'>&gt; Allowance replinshed.</p>"
        );
      })();
    }
